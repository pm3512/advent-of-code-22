...
impl Rock {
    fn is_valid_pos(&self, x: i32, y: i32, chamber: &Vec<[bool; CHAMBER_WIDTH as usize]>) -> bool {
        if y < 0 || x < 0 {
            return false;
        }
    ...
    fn jet_step(&mut self, dir: u8, chamber: &Vec<[bool; CHAMBER_WIDTH]>) {
        if dir == b'<' {
            if self.is_valid_pos(self.x as i32 - 1, self.y as i32, chamber) {
                self.x -= 1;
            }
        } else {
            assert!(dir == b'>');
    ...
    fn fall_step(&mut self, chamber: &mut Vec<[bool; CHAMBER_WIDTH as usize]>) -> bool {
        if self.is_valid_pos(self.x as i32, self.y as i32 - 1, chamber) {
            self.y -= 1;
            true
        } else {
            let y = self.y;
...
pub fn solve() {
    ...
    for i in 0..NUM_ITER {
        let mut rock = Rock::new(max_height, i);
